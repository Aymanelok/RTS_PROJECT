...
class SensorNode(Node):
    ...
    def __init__(self):
        ...
        # ── Abonnement à la Simulation Réelle ─────────────────
        self.create_subscription(String, "/sim_counts", self._on_sim_counts, RT_QOS)
        self._sim_counts = None
    ...
    # ─────────────────────────────────────────────────────────
    # Callback données simulation
    # ─────────────────────────────────────────────────────────
    def _on_sim_counts(self, msg: String) -> None:
        try:
            d = json.loads(msg.data)
            self._sim_counts = d.get("counts")
            self._last_sim_time = time.monotonic()
        except Exception as e:
            self.get_logger().warn(f"[SensorNode] Erreur sim_counts: {e}")
    ...
    def _timer_callback(self) -> None:
        ...
        # Si on a reçu des données de la simulation il y a moins d'une seconde, on les utilise !
        if self._sim_counts and (time.monotonic() - self._last_sim_time < 1.0):
            counts = self._sim_counts
        else:
            # Fallback sur la génération mathématique synthétique
